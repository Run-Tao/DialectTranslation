...
寻找方言集合$\mathbb{D}$ 与普通话集合$\mathbb{M}$的映射关系，即同一指令的方言语音特征矩阵$M_d$与普通话语音特征矩阵$M_m$之间的映射关系。寻找权重矩阵$\Omega_{D\rightarrow M}$，使其满足：
\begin{equation}
    M_m = \Omega_{\mathbb{D}\rightarrow  \mathbb{M}} \odot M_d
\end{equation}
对
\subsection{预测}
...
\begin{align}
    D_p = F(M_{SVD_p}) &= \Omega_{\mathbb{D}\rightarrow  \mathbb{M}} \odot M_{SVD_p} \\
    &= \begin{bmatrix}
    ...
    \end{bmatrix}\\
    &= \begin{bmatrix}
        \omega_{1,1}m_{1,1} & \omega_{1,1}m_{1,2} & \cdots & \omega_{1,1}m_{1,N} \\
        \omega_{2,1}m_{2,1} & \omega_{2,1}m_{2,2} & \cdots & \omega_{2,1}m_{2,N} \\
        \vdots & \vdots & \ddots & \vdots \\
        \omega_{L,1}m_{L,1} & \omega_{L,1}m_{L,2} & \cdots & \omega_{L,1}m_{L,N}\\
    \end{bmatrix}
\end{align}
